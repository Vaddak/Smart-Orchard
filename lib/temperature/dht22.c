...
int DHTgpio = 17;                // my default DHT pin = 4
float humidity = 0.;
float temperature = 0.;
...
int getSignalLevel( int usTimeOut, bool state )
{
   ...
   while( gpio_get_level(DHTgpio)==state ) {
       if( uSec > usTimeOut ) 
           return -1; // Error de tiempo de espera
...
int readDHT()
{
   ...
   //ESP_LOGI( TAG, "Response 1 = %d", uSec );
   if( uSec < 0 ) return DHT_TIMEOUT_ERROR; 

   // -- 80us up ------------------------
   uSec = getSignalLevel( 85, 1 );
   // ESP_LOGI( TAG, "Response 2 = %d", uSec );
   if( uSec < 0 ) return DHT_TIMEOUT_ERROR;
   ...
   for( int k = 0; k < 40; k++ ) {
       ...
       // Esperar hasta que el sensor cambie a alto
       uSec = 0;
       while( gpio_get_level(DHTgpio) == 1 ) {
           if( uSec > 100 ) // Tiempo de espera máximo de 100 microsegundos
       ...
       // since all dhtData array were set to 0 at the start, 
